...
const CikarmaPage = () => {
  ...
  const checkAnswer = () => {
    ...
    if (userAnswer === num1 - num2) {
      showSuccess("Aferin! Doğru cevap!");
      setIsCorrect(true);
      setTimeout(() => {
        setNum1(Math.floor(Math.random() * 10) + 1);
        setNum2(Math.floor(Math.random() * num1) + 1);
        setAnswer("");
        setIsCorrect(null);
      }, 1500);
    } else {
      showError("Hadi tekrar deneyelim!");
      setIsCorrect(false);
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
          ...
          {isCorrect !== null && (
            <div className={`text-center text-2xl font-bold mb-4 ${isCorrect ? 'text-green-600' : 'text-red-600'}`}>
              {isCorrect ? <Check className="inline mr-2" /> : <X className="inlinemr-2" />}
