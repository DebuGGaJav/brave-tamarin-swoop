...
const CarpmaPage = () => {
  ...
  const generateNumbers = () => {
    let max = difficulty === "easy" ? 5 : difficulty === "medium" ? 10 : 12; // Max for multiplication
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-50 to-yellow-50 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8 sm:mb-10">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-orange-600 mb-2">Çarpma İşlemleri</h1>
          <p className="text-lg sm:text-xl text-gray-600">Eğlenceli çarpma öğrenelim!</p>
        </div>
        ...
        <Card className="mb-4 shadow-xl border-2 border-orange-200">
          <CardHeader>
            <CardTitle className="text-center text-orange-600 text-xl sm:text-2xl">Çarpma Sorusu</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-center mb-6">
              <img src="/images/pencil.png" alt="Pencils" className="mx-auto mb-4 w-24 h-24 sm:w-32 sm:h-32 object-contain" />
              <div className="flex justify-center items-center space-x-3 sm:space-x-4 mb-6">
                <div className="text-3xl sm:text-4xl font-bold text-orange-600">{num1}</div>
                <X className="w-6 h-6 sm:w-8 sm:h-8 text-orange-600" />
                <div className="text-3xl sm:text-4xl font-bold text-orange-600">{num2}</div>
                <div className="text-3xl sm:text-4xl font-bold text-gray-400">=</div>
                <input
                  type="number"
                  value={userAnswer}
                  onChange={(e) => setUserAnswer(e.target.value)}
                  className="w-24 h-12 sm:w-32 sm:h-16 text-xl sm:text-2xl text-center border-2 border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
                  placeholder="?"
                />
              </div>
        ...
        <ProgressTracker topic="Çarpma" correctAnswers={correctAnswers} totalQuestions={totalQuestions} />
        
        <div className="mt-8 text-center">
          <h2 className="text-xl sm:text-2xl font-bold text-orange-600">Toplam Puan: {totalPoints}</h2>
          {totalPoints >= 50 && (
