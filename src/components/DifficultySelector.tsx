        ...
        <div className="grid grid-cols-3 gap-3">
          {levels.map((level) => (
            <Button
              ...
              className={`flex flex-col items-center p-2 sm:p-3 md:p-4 h-auto space-y-1 sm:space-y-2 transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl text-wrap ${ // text-wrap eklendi
                currentLevel === level.id
                  ? `${level.color} ${level.borderColor} border-2 ring-2 ring-offset-2`
                  : "bg-gray-100 hover:bg-gray-200 border-2 border-gray-300 text-gray-800"
              } ${level.hoverColor}`}
            >
              <div className="p-1 sm:p-2 rounded-full bg-white/20 backdrop-blur-sm">
                {level.icon}
              </div>
              <span className="text-sm sm:text-base font-bold text-center leading-tight">{level.name}</span> {/* leading-tight eklendi */}
              <span className="text-xs sm:text-sm opacity-90 text-center leading-tight">{level.description}</span> {/* leading-tight eklendi */}
            </Button>
          ))}
        </div>
      </CardContent>
